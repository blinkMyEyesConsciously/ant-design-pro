import {Divider, Button, Tree, Card, Skeleton} from "antd";
...
const UserList: React.FC<{}> = () => {
    ...
    const {data: menuTreeList, loading: menTreeLoading, refresh} = useRequest<any[]> (getMenuTreeMenuList);
    ...
    const columns: ProColumns<MenuEntity>[] = [
...

        {
            ...
            render: (_, record) => (
                <>
                    <ButtonSubmit okCallback={ () => {
                        actionRef.current?.reload ()
                        refresh ()

                    } } loadingContent={ '删除中' } successContent={ "删除完成" } reqFun={ deleteMenuById }
                                  data={ {id: record.menuId} }>删除</ButtonSubmit>
    ...
    return (
        <PageContainer>
            <AddOrUpDateMenu menuItem={ menuItem } show={ addMenuShow } onCancel={ () => {
                addMenuShowSetFalse ()

            } } type={ addOrUpdateMenuType } onOk={
                () => {
                    addMenuShowSetFalse ()
                    actionRef.current?.reload ()
                    refresh ()
            ...
            <ProTable<TableListItem>
                ...
                request={ (params) => {
                    return pageTransition<TableListItem> ((obj) => getMenu ({
                        ...obj,
                        menuType: 1,
